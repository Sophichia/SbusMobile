        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                ...
                for (int i = 0; i < vehicleArray.length(); i++) {
                    ...
                    if (!jVehicle.isNull(0)) v.setTripId(jVehicle.getString(0));
                    if (!jVehicle.isNull(1)) v.setRouteId(jVehicle.getString(1));
                    if (!jVehicle.isNull(2)) v.setServiceId(jVehicle.getString(2));
                    if (!jVehicle.isNull(3)) v.setShapeId(jVehicle.getString(3));
                    if (!jVehicle.isNull(4)) v.setRouteShortName(jVehicle.getString(4));
                    if (!jVehicle.isNull(5)) v.setRouteLongName(jVehicle.getString(5));
                    if (!jVehicle.isNull(6)) v.setRouteColor(jVehicle.getString(6));
...
//                            Log.d("Stop", jStop.toString());
                            Stop s = new Stop();
                            if (!jStop.isNull(0)) s.setId(jStop.getString(0));
                            if (!jStop.isNull(1)) s.setStopHeadsign(jStop.getString(1));
                            if (!jStop.isNull(2)) s.setStopSequence(jStop.getInt(2));
                            if (!jStop.isNull(3)) s.setName(jStop.getString(3));
                            if (!jStop.isNull(4)) s.setLatitude(jStop.getDouble(4));
                            if (!jStop.isNull(5)) s.setLongitude(jStop.getDouble(5));
                            if (!jStop.isNull(6)) s.setArrivalTime(jStop.getString(6));
                            stops.add(s);
                        }
                        v.setStops(stops);
                    }
        ...
        while (reader.hasNext()) {
            ...
            if (reader.peek() != JsonToken.NULL) {
                if (tag.equals(TAG_ROUTE_ID)) {
                    v.setRouteId(reader.nextString());
                } else if (tag.equals(TAG_SERVICE_ID)) {
                    v.setServiceId(reader.nextString());
                } else if (tag.equals(TAG_SHAPE_ID)) {
                    v.setShapeId(reader.nextString());
                } else if (tag.equals(TAG_TRIP_ID)) {
                    v.setTripId(reader.nextString());
                } else if (tag.equals(TAG_ROUTE_LONG_NAME)) {
                    v.setRouteLongName(reader.nextString());
                } else if (tag.equals(TAG_ROUTE_SHORT_NAME)) {
                    v.setRouteShortName(reader.nextString());
                } else if (tag.equals(TAG_STOP_HEADSIGN)) {
                    v.setStopHeadsign(reader.nextString());
                } else if (tag.equals(TAG_STOPS)) {
                    v.setStops(readVehicleStopsArray(reader));
                } else {
                    reader.skipValue();
                }
            } else {
